...
LocalVariables::LocalVariables (uint16_t maxSize) : max(maxSize), dois(false), realMax(2*maxSize) {
	elements = (uint32_t *) calloc(max*2, sizeof(uint32_t));
	types = (uint8_t *) calloc(max*2, sizeof(uint8_t));
}

LocalVariables::LocalVariables (uint16_t maxSize, bool slots) : max(maxSize), dois(slots), realMax(2*maxSize) {
	elements = (uint32_t *) calloc(max*2, sizeof(uint32_t));
	types = (uint8_t *) calloc(max*2, sizeof(uint8_t));
}
...
typedElement LocalVariables::get(int index) const {
	...
	if (this->types[index] == TYPE_LONG || this->types[index] == TYPE_DOUBLE || (this->types[index] == TYPE_REFERENCE && BITS)) {
		ret.value.l = (long(this->elements[index+1]) << 32) + this->elements[index];
	} else {
