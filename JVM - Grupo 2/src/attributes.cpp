...
//funcao para ler um atributo
// attribute_info readAttribute (FILE* fp) {
//  attribute_info ret;

//  ret.name_index = readU2(fp);
//  ret.length = readU4(fp);
//  ret.info = (unsigned char *) malloc(sizeof(unsigned char) * ret.length);
//  for (int i = 0; i < ret.length; i++) {
//    ret.info[i] = readU1(fp);
//  }
...
t_exception_table* readExceptionHandler(FILE* fp) {
  ...
  t_exception_table* ret = (t_exception_table*) malloc(sizeof(t_exception_table));

  ret->start_pc = readU2(fp);
  ret->end_pc = readU2(fp);
  ret->handler_pc = readU2(fp);
  ret->catch_type = readU2(fp);
...
t_info* readAttributeInfo(FILE* fp, cp_info *cp, unsigned short index, unsigned short length) {
  ...
  string attribute_name = dereferenceIndex(cp, index);

  if(attribute_name == "ConstantValue") {
    info->constantvalue.constantvalue_index = readU2(fp);
  }

  else if(attribute_name == "Code") {
    info->code.max_stack = readU2(fp);
    info->code.max_locals = readU2(fp);
    info->code.code_length = readU4(fp);

    unsigned char* code_list = (unsigned char*) malloc(sizeof(t_exception_table) * info->code.code_length);
    for(int i = 0; i < info->code.code_length; i++ ) {
      code_list[i] = readU1(fp);
    }
    info->code.code = code_list;    

    info->code.exception_table_length = readU2(fp);
    ...
    for(int i = 0; i < info->code.exception_table_length; i++ ) {
      e_table[i] = readExceptionHandler(fp);
    }
    info->code.exception_table = e_table;

    info->code.attribute_count = readU2(fp);
    ...
    for(int i = 0; i < info->code.attribute_count; i++ ) {
      attributes[i] = readAttribute(fp, cp);
    }
    info->code.attributes = attributes;
  }

  else if(attribute_name == "Exceptions") {
    info->exception.number_of_exceptions = readU2(fp);
...
attribute_info readAttribute (FILE* fp, cp_info *cp) {
  ...
  ret.info = readAttributeInfo(fp, cp, ret.name_index, ret.length);
...
void showAttribute (attribute_info a, cp_info *cp) {
  ...
  if(attribute_name == "ConstantValue") {
    ...
    while(i < a.info->code.code_length) {
      cout << "\t\tCode #" << i << ":" << getMnemonic(a.info->code.code[i]);
    ...
    for(int i = 0; i < a.info->code.exception_table_length; i++ ) {
      cout << "\t\tStart   PC: " << a.info->code.exception_table[i]->start_pc << endl;
      cout << "\t\tEnd     PC: " << a.info->code.exception_table[i]->end_pc << endl;
      cout << "\t\tHandler PC: " << a.info->code.exception_table[i]->handler_pc << endl;
      cout << "\t\tCatch Type: " << a.info->code.exception_table[i]->catch_type << endl;
    }

    cout << "\t\tAttribute Count: " << a.info->code.attribute_count << endl;

    for(int i = 0; i < a.info->code.attribute_count; i++ ) {
      showAttribute(a.info->code.attributes[i],cp);
    }
  }
    ...
    for(int i = 0; i < a.info->exception.number_of_exceptions; i++ ) {
      ...
      cout << "\t\tException Index: " << a.info->exception.exception_index_table[i] << endl; 
    }
  }
}
...
string getMnemonic(int opcode) {
  switch(opcode) {
    ...
    // depois mudar para mostrar o erro
    default:    return NULL;
  }
}
